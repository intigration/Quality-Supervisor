...
def send_messages():
    ...
    chats = int(
        input("Enter the number of personal chats/groups you would want to message "))

    for chat in range(chats):

        # This is used to enable the user to search for the contact
        driver.find_element_by_xpath(
            '/html/body/div[1]/div/div/div[3]/div/header/div[2]/div/span/div[2]/div').click()
        ...
        driver.find_element_by_xpath(
            '/html/body/div[1]/div/div/div[2]/div[1]/span/div/span/div/div[1]/div/label/div/div[2]').send_keys(name)
        time.sleep(three)

        # The contact's chat will be opened
        driver.find_element_by_xpath(
            '/html/body/div[1]/div/div/div[2]/div[1]/span/div/span/div/div[1]/div/label/div/div[2]').send_keys(Keys.ENTER)
        ...
        while done:
            ...
            driver.find_element_by_xpath(
                '/html/body/div[1]/div/div/div[4]/div/footer/div[1]/div[2]/div/div[2]').send_keys(message)
            time.sleep(three)
