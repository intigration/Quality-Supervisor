...
def scraper():
    ...
    while 1:
        ...
        driver.get(f"https://clutch.co/{query}")
        ...
        for every in store_urls:
            try:
                ...
                print(header1.text)
                # get the star rating
                star = driver.find_element_by_class_name("bARER")
                each_info.append(star.text)
                print(star.text)
                # get the number of comments
                comments = driver.find_element_by_class_name("EHUI5b")
                each_info.append(comments.text.split()[0])

                # get the footer information like installs, version, email, etc.
                stat_info_table = driver.find_elements_by_class_name("w7Iutd")
                stats = []
                for x in range(len(stat_info_table)):
                    if x % 2 == 0:
                        stats.append(stat_info_table[x].text)

                stat_header = driver.find_elements_by_class_name("wVqUob")
